...
function killPort3000() {
    if (os.platform() === "darwin" || os.platform() === "linux") {
        // MacOS hoặc Linux
        exec("lsof -i :3000 | grep LISTEN | awk '{print $2}' | xargs kill -9", (error, stdout, stderr) => {
            if (error) {
                console.error(`Lỗi khi dừng cổng 3000 trên macOS/Linux: ${error.message}`);
                return;
            }
        ...
        exec("netstat -aon | findstr :3000", (error, stdout, stderr) => {
            if (error) {
                console.error(`Lỗi khi tìm kiếm cổng 3000 trên Windows: ${error.message}`);
                return;
            }
            ...
            lines.forEach(line => {
                ...
                exec(`taskkill /F /PID ${pid}`, (killError, killStdout, killStderr) => {
                    if (killError) {
                        console.error(`Lỗi khi dừng cổng 3000 trên Windows: ${killError.message}`);
                        return;
                    }
